...
WITH conta_dep_tema AS (
  ...
  -- Tabela que conta, para cada parlamentar, o número de intervenções (proposições) associadas a cada tema:
  SELECT 
    pt.id_deputado, pt.ultimoStatus_nome, pt.sigla_partido, pt.uf, pt.tema, COUNT(DISTINCT pt.id_proposicao_tema) AS n_intervencoes, n_anos_atuacao
  FROM prop_tema AS pt
  WHERE pt.ultima_legislatura = 56
  GROUP BY pt.id_deputado, pt.ultimoStatus_nome, pt.sigla_partido, pt.uf, pt.tema, pt.n_anos_atuacao
)

-- Adiciona à Tabela acima uma coluna com o número total de intervenções de cada deputado:
SELECT 
  id_deputado, ultimoStatus_nome, sigla_partido, uf, tema, n_intervencoes, 

...
SELECT 
  ...
  -- Info do cargo:
  cargos.idCargo AS id_cargo,
  cargos.nomeCargo AS nome_cargo,
  PARSE_DATE('%d/%m/%Y', cargos.dataEntrada) AS data_entrada,
  PARSE_DATE('%d/%m/%Y', cargos.dataSaida) AS data_saida,
  -- Info da comissão:
  cargos.idOrgaoLegislativoCD AS id_orgao_cd,
  cargos.siglaComissao AS sigla_comissao,
  cargos.nomeComissao AS nome_comissao,
...
UNNEST(d.cargosComissoes.cargoComissoes) AS cargos

/*** 
     ...
     os blocos. No final, colocamos informações sobre as proposições em questão.
...
SELECT
  ...
  l.id AS id_proposicao, l.ementa, l.id_deputado AS id_autor, l.nome_autor, l.sigla_partido_autor, l.sigla_uf_autor AS uf_autor,
  l.url_inteiro_teor,
  -- Info do partido:
  q AS sigla_partido,
...
LEFT JOIN `gabinete-compartilhado.congresso.proposicoes_completa` AS l
ON p.sigla_tipo = l.sigla_tipo AND p.ano = l.ano AND p.numero = l.numero

GROUP BY p.timestamp, p.cod_sessao, p.sigla_tipo, p.numero, p.ano, p.obj_votacao, p.resumo, sigla_partido,
  id_proposicao, ementa, id_autor, nome_autor, sigla_partido_autor, uf_autor, url_inteiro_teor

...
WITH total_receitas_partidarias AS (
  SELECT
    ...
    ANY_VALUE(numero_candidato) AS numero_urna_candidato,
    ANY_VALUE(cargo) AS cargo,
    ANY_VALUE(cpf_candidato) AS cpf_candidato_str,
    ANY_VALUE(uf) AS sigla_unidade_eleitoral_candidato,
    ANY_VALUE(sigla_partido) AS sigla_partido_candidato,
    -- Valor total recebido de partidos:
    SUM(IF(tipo_receita = 'Pública' AND NOT (
      -- Exclui doações de comitês de outros candidatos:
      nome_doador_rfb_unico LIKE '%GOVERNADOR%' OR
     (nome_doador_rfb_unico LIKE '%DEPUTADO%' AND nome_doador_rfb_unico NOT LIKE '%FINANCEIRO%') OR
     (nome_doador_rfb_unico LIKE '%ELEICAO%SENADOR%' AND nome_doador_rfb_unico NOT LIKE '%FINANCEIRO%') OR
      nome_doador_rfb_unico LIKE '%ELEICAO%PRESIDENTE%'), valor_receita_unico, 0)) AS valor_candidato 

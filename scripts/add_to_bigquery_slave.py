...
read_from_aws = False
# Number of files to use to deduce schema:
n_files_schema = 500
...
def save_raw_data_to_local_GCP(temp_data, bucket, prefix, select_proba, seed):
    """
    ...
    If `seed` (int) > 0 or None, sample the files randomly 
    with probability `select_proba` (float), in the order 
    they appear. If `seed` < 0, really use the first 500 files.
    """
    ...
    for i, obj in enumerate(blob_iterator):
        # Select object by chance (or not, if seed < 0):
        rand = random.uniform(0, 1)
        if (seed != None and seed < 0) or rand > select_proba:
            n_examples = n_examples + 1
            if n_examples > n_files_schema:
                break
...
def save_raw_data_to_local_AWS(temp_data, bucket, prefix, select_proba, seed):
    """
    Save the first 500 entries (i.e. files) in the database to a temp file
    to use it to build a schema for the data. 
...
def save_raw_data_to_local_local(temp_data, bucket, prefix, max_bytes, select_proba, seed):
    """
    Save the first 500 files found locally to a temp file
    to use it to build a schema for the data. 
...
def save_raw_data_to_local(temp_data, bucket, prefix, max_bytes, select_proba, seed):
    """
    ...
    if bucket == 'local':
        save_raw_data_to_local_local(temp_data, bucket, prefix, max_bytes, select_proba, seed)

    else:
        if read_from_aws == True:
            save_raw_data_to_local_AWS(temp_data, bucket, prefix, select_proba, seed)
        else:
            save_raw_data_to_local_GCP(temp_data, bucket, prefix, select_proba, seed)
...
def lambda_handler(event, context):
    ...
    save_raw_data_to_local(RAW_DATA, event['bucket_name'], event['prefix'], event['max_bytes'], event['select_proba'], event['seed'])

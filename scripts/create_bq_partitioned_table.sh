...
# Hard-coded to build the EXTERNAL TABLE  'cadastro_reserva_reforma' in dataset 'executivo_federal_servidores'
schema=`head -n1 ~/gabinete/projetos/fiscalizacao/dados/servidores-executivo-federal/limpos/v2/202001_Reserva_Reforma_Militares/202001_Cadastro_cleaned.csv | sed -e 's/,/:STRING,/g' -e '1s/^\xEF\xBB\xBF//' | awk '{print $1":STRING"}'`
bq --project_id=gabinete-compartilhado mkdef --noautodetect --source_format=CSV --hive_partitioning_mode=CUSTOM --hive_partitioning_source_uri_prefix=gs://brutos-publicos/executivo/federal/servidores/partitioned_v2/Reserva/{data:DATE} gs://brutos-publicos/executivo/federal/servidores/partitioned_v2/Reserva/*_Cadastro_cleaned.csv ${schema} | sed 's/"skipLeadingRows": 0/"skipLeadingRows": 1/g' > table_definition_files/servidores_federais_cadastro_reserva_reforma.json
bq --project_id=gabinete-compartilhado mk --external_table_definition=table_definition_files/servidores_federais_cadastro_reserva_reforma.json executivo_federal_servidores.cadastro_reserva_reforma

...
FROM (SELECT get_scheduled_queries.py list_scheduled_queries.sh list_tables.py FROM `gabinete-compartilhado.congresso.tramitacao_` WHERE data_hora BETWEEN DATETIME '2019-02-01' AND DATETIME '2019-12-31') t1
JOIN (SELECT get_scheduled_queries.py list_scheduled_queries.sh list_tables.py FROM `gabinete-compartilhado.congresso.camara_deputado_` WHERE ultima_legislatura = 56 ORDER BY nome_parlamentar) t2
